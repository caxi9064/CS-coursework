...
def do_color_filtering(img):
  ...
  for i in range(img_height): 
    for j in range(img_width):
      #pixel = mask[i][j]
      if check_if_color_in_range(img[i,j]) == True: # if in range
        mask[i,j] = 1
  return mask
...
def get_blobs(img_mask):
  ...
  copy_img_mask = copy.copy(img_mask)

  for i in range(img_mask_height): # TODO: Iterate through all 'y' coordinates in img_mask
      for j in range(img_mask_width): # TODO: Iterate through all 'x' coordinates in img_mask
        if img_mask[i,j] == 1:  # TODO: If mask value at [y,x] is 1, call expand_nr on copy of image mask and coordinate (y,x), giving a third argument of an empty list to populate with blob_coords.
          # blob_coords = []
          # expand_nr(copy_img_mask, (i,j), blob_coords)
          # blobs_list.append(blob_coords)
          coords_in_blob = expand_nr(copy_img_mask, (i,j), [])
          blobs_list.append(coords_in_blob) # TODO: Add blob_coords to blobs_list
...
def get_blob_centroids(blobs_list):
  ...
  for list in blobs_list:
    #print(list)
    if len(list) > 600:
      centroid = np.mean(np.array(list),axis=0)
      #print(centroid)
