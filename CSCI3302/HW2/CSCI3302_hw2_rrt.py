...
def get_nearest_vertex(node_list, q_point):
    ...
    closestNode = first_node
    dist = np.sqrt(np.sum(np.square(np.subtract(np.array(first_node.point), np.array(q_point))))) # computing dist between points
    smallest = dist

    for node in node_list:
        dist = np.sqrt(np.sum(np.square(np.subtract(np.array(node.point), np.array(q_point))))) # computing dist between points
        if dist <= smallest: # if the dist is smaller than update closest to q_point
            closestNode = node
            smallest = dist
...
def steer(from_point, to_point, delta_q):
    ...
    '''
    dist = np.sqrt(np.sum(np.square(np.subtract(np.array(from_point), np.array(to_point))))) # computing dist between points
    c = dist/delta_q # c is the distance between from_point and to_point divided by delta_q

    if dist > delta_q: # need to move it since distance exceeds delta_q
        for i in range(len(to_point)): # for each dimension(e.g x,y,z,etc.)
            to_point[i] = (((from_point[i]-to_point[i])/c) + from_point[i]) # updated to_point dimension = ((to_point dimension - from_point dimension)/c) + from_point dimension
       
    # TODO Use the np.linspace function to get 10 points along the path from "from_point" to "to_point"
    temp = np.array([np.linspace(x, y, 10) for x, y in zip(np.array(from_point), np.array(to_point))])
    path = []
    for column in range(len(temp[0])):
        point = []
        for row in range(len(temp)):
            point.append(temp[row][column])
        path.append(point)
...
def check_path_valid(path, state_is_valid):
    ...
    '''
    for point in path:
        if (state_is_valid(point) == False): # collision
            return False
    return True
...
def rrt(state_bounds, state_is_valid, starting_point, goal_point, k, delta_q):
    ...
    if goal_point is None: # build graph and terminate after k iterations
        for i in range(k):
            q_rand = get_random_valid_vertex(state_is_valid, state_bounds)
            q_near_node = get_nearest_vertex(node_list, q_rand) # get closest node to rand
            path_nearest_to_rand = steer(q_near_node.point, q_rand, delta_q)
            q_rand = path_nearest_to_rand[-1] # q_rand is within dist of parent  
    
            dist = np.sqrt(np.sum(np.square(np.subtract(np.array(q_near_node.point), np.array(q_rand)))))
            c = dist/delta_q # c is the distance between q_rand and q_near divided by delta_q
            q_new = (np.subtract(q_rand,q_near_node.point)/c) + q_near_node.point


            node = Node(q_new, q_near_node)
            path = steer(q_near_node.point, q_new, delta_q)
            if (check_path_valid(path, state_is_valid)): # if path is valid
                node_list.append(node)
                node.path_from_parent = path
        return np.array(node_list)

    else: # return once goal is found
        for i in range(k):
            q_rand = get_random_valid_vertex(state_is_valid, state_bounds)
            q_near_node = get_nearest_vertex(node_list, q_rand) # get closest node to rand
            path_nearest_to_rand = steer(q_near_node.point, q_rand, delta_q)
            ...
            if random.random() < 0.05: 
                q_rand = goal_point
            else: q_rand = path_nearest_to_rand[-1] # q_rand is within dist of parent  

            dist = np.sqrt(np.sum(np.square(np.subtract(np.array(q_near_node.point), np.array(q_rand)))))
            c = dist/delta_q # c is the distance between q_rand and q_near divided by delta_q
            # for i in range(len(q_near_node.point)): # for each dimension(e.g x,y,z,etc.)
            #     q_new.append(((q_rand[i]-q_near_node.point[i])/c) + q_near_node.point[i])

            q_new = (np.subtract(q_rand,q_near_node.point)/c) + q_near_node.point

            new_node = Node(q_new, q_near_node)
            path = steer(q_near_node.point, q_new, delta_q)
            if (check_path_valid(path, state_is_valid)): # if path is valid
                node_list.append(new_node)
                new_node.path_from_parent = path

                dist = np.sqrt(np.sum(np.square(np.subtract(np.array(q_new), np.array(goal_point)))))
                if dist < np.e**-5: # if q_new.point is within distance
                    return np.array(node_list)
        return np.array(node_list)
