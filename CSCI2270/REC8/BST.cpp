...
return currNode;//function will return this so parent node points to new node
}
...
// GOLD TODO
bool traverse(Node* n, Node* min, Node* max)
{
    if (n == NULL)
    {
        return true;
    }
    if (n->key < min->key || n->key > max->key)
    {
        return false;
    }
    else//recursively compare until base case is reached or tree is invalid
    {
        traverse(n->left, min, max);
        traverse(n->right, min, max);   
    }
    return true;
}
bool BST::isValidBST()
{
  ...
  {
    return traverse(root, getMinValueNode(root),getMaxValueNode(root));
  }
}
