...
int Graph::findShortestPath(int src, int dest){
    ...
    while(!q.empty())
    {
        ...
        for( int x = 0; x < n->adj.size(); x++ )
        {
            ...
            if (n->adj[x].v->visited == false) // n is pointing to vertex structure, then accessing adj[] and each adjacent vertex of v
            {
                n->adj[x].v -> visited = true;
                q.push(n->adj[x].v);
                n->adj[x].v -> distance = n->distance + 1;// Update the distance of the adjacent vertices along the way
            }
